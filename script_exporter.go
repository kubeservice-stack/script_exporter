...
func (h *handler) innerHandler(filters ...string) (http.Handler, error) {
	...
	r.MustRegister(versioncollector.NewCollector("script_exporter"))
...
func main() {
	...
	logger.Info("msg", "Starting script_exporter", "version", version.Info())
	logger.Info("msg", "Build context", "build_context", version.BuildContext())
	if user, err := user.Current(); err == nil && user.Uid == "0" {
		logger.Warn("msg", "Node Exporter is running as root user. This exporter is designed to run as unprivileged user, root is not required.")
	}
